  ...
  const startMatch = (round: 'semifinal' | 'final', matchIndex: number) => {
    ...
    navigate('/game', {
      state: {
        ...
        player1:
          round === 'semifinal'
            ? semifinals[matchIndex].player1
            : finalMatch.player1,
        player2:
          round === 'semifinal'
            ? semifinals[matchIndex].player2
            : finalMatch.player2,
        returnTo: '/tournament',
      },
    })
  ...
  const handleMatchComplete = (
    ...
    if (round === 'semifinal') {
      ...
      if (updatedSemifinals[otherMatchIndex].winner) {
        const finalist1 = {
          name:
            updatedSemifinals[0].winner === updatedSemifinals[0].player1.name
        ...
        const finalist2 = {
          name:
            updatedSemifinals[1].winner === updatedSemifinals[1].player1.name
            ...
            {semifinals.map((match, index) => (
              <div key={index} className="match-card semifinal">
                ...
                {match.status === 'in_progress' && (
                  <div className="match-status">Match in Progress...</div>
                )}
                {match.status === 'completed' && match.winner && (
                  <div className="match-result">Winner: {match.winner}</div>
                )}
              </div>
            ))}
          </div>
          ...
          {finalMatch.player1.name && finalMatch.player2.name && (
            <div className="match-card final">
              ...
              <div className="match-players">
                <div className="player">
                  <img
                    src={finalMatch.player1.avatar}
                    alt={finalMatch.player1.name}
                    className="player-avatar"
                  />
                ...
                <div className="player">
                  <img
                    src={finalMatch.player2.avatar}
                    alt={finalMatch.player2.name}
                    className="player-avatar"
                  />
              ...
              {finalMatch.status === 'in_progress' && (
                <div className="match-status">Final Match in Progress...</div>
              )}
              {finalMatch.status === 'completed' && finalMatch.winner && (
                <div className="match-result">Winner: {finalMatch.winner}</div>
              )}
            </div>
          )}
